...
def is_primitive_root(a, p, phi_factors):
    """Check if a is a primitive root mod p given prime factors of phi=p-1."""
    phi = p - 1
...
def smallest_prime_primitive_root(p, is_prime_arr, phi_factors):
    """Find smallest prime q < p that is a primitive root mod p."""
    q = 2
    while q < p:
        if is_prime_arr[q]:
...
print(f"Cases where smallest prime prim root q > p/2: {len(worst_cases)}")
print(f"Worst cases (top 20 by ratio):")
