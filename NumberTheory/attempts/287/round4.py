...
while stack:
    ...
    if slots_left * Fraction(1, min_val) < remaining:
        continue
    # n >= ceil(1/remaining) to ensure 1/n <= remaining
    lo = math.ceil(remaining.denominator / remaining.numerator)
    start = max(min_val, lo)
    # upper bound: n <= floor(slots_left / remaining)
...
if reps_with_gaps:
    ...
    print('Top 15 with smallest max gap:')
    for mg, rep, gaps in reps_with_gaps[:15]:
        print('  max_gap=%d rep=%s gaps=%s' % (mg, rep, gaps))